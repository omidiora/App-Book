  ...
  try {
    ...
    trialPeriodRefs.once('value', async snapshot => {
      ...
      try {
        ...
        dispatch(SetCurrentUser({ userId: user.uid, name: data.name, email: data.email }));
        resolve(data);
...
export const Login = data => dispatch => new Promise(async (resolve, reject) => {
  try {
    ...
    dispatch(SetCurrentUser({ userId, name, email }));
...
export const UserExist = email => new Promise((resolve, reject) => {
  const usersRefs = app.database().ref('/users');
  usersRefs
    .orderByChild('email')
    .equalTo(email)
    .once('value', snapshot => {
     ...
     console.log(value, 'adasdjasbdjas')
    });
});
