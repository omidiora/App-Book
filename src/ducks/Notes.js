...
export const SaveNotesOffline = (courseId, notes) => dispatch => new Promise(async (resolve, reject) => {
  ...
  try {
    ...
    // const getNoteSize = note => fetch(note.noteUrl, { method: 'HEAD', headers: { 'Content-Type': 'Application/json' } });
    // await Promise.all(notes.map(async note => {
    //   const head = await getNoteSize(note);
    //   console.log(head, 'next');

    //   console.log(head.headers.get('Content-Length'), 'length');
